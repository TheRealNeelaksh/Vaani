...
def read_audio(path: str, sampling_rate: int = 16000, normalize=False):
...
class SileroVadDataset(Dataset):
    def __init__(self, config, mode="train"):
        ...
        dataset_path = (
            config.train_dataset_path if mode == "train" else config.val_dataset_path
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.resample_to_8k:
            transform = torchaudio.transforms.Resample(orig_freq=self.sr, new_freq=8000)
            wav = transform(wav)
    ...
    def load_speech_sample(self, idx=None):
        ...
        gt, mask = self.get_ground_truth_annotated(
            self.index_dict[idx]["speech_ts"], len(wav)
        )
...
def train(config, loader, jit_model, decoder, criterion, optimizer, device):
    ...
    encoder_layer = (
        jit_model._model_8k.encoder if config.tune_8k else jit_model._model.encoder
    )
...
def validate(config, loader, jit_model, decoder, criterion, device):
