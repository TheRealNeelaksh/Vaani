...
if __name__ == "__main__":
    ...
    loader = torch.utils.data.DataLoader(
        SileroVadDataset(config, mode="val"),
    ...
    all_predicts, all_gts = predict(
        model, loader, config.device, sr=8000 if config.tune_8k else 16000
    )
    print("Calculating thresholds...")
    best_ths_enter, best_ths_exit, best_acc = calculate_best_thresholds(
        all_predicts, all_gts
    )
    print(
        f"Best threshold: {best_ths_enter}\nBest exit threshold: {best_ths_exit}\nBest accuracy: {best_acc}"
    )
