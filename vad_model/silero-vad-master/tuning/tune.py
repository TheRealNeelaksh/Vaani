...
if __name__ == "__main__":
    ...
    decoder.load_state_dict(
        model._model_8k.decoder.state_dict()
        if config.tune_8k
        else model._model.decoder.state_dict()
    )
    decoder.train()
    params = decoder.parameters()
    optimizer = torch.optim.Adam(
        filter(lambda p: p.requires_grad, params), lr=config.learning_rate
    )
    ...
    for i in range(config.num_epochs):
        ...
        train_loss = train(
            config, train_loader, model, decoder, criterion, optimizer, config.device
        )
        val_loss, val_roc = validate(
            config, val_loader, model, decoder, criterion, config.device
        )
        print(
            f"Metrics after epoch {i + 1}:\n" f"\tTrain loss: {round(train_loss, 3)}\n",
